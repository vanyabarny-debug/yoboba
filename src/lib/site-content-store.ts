...
export const default_top_bar_links: top_bar_link[] = [
  ...
  { id: 'jobs', label: 'работа в yoboba', href: '/rabota', is_active: true },
...
export const default_pages: site_page[] = [
  {
    slug: 'o-nas',
    title: 'о нас',
    body: `yoboba — это больше, чем бабл-ти. это место с огромным выбором креативных напитков, хрустящими корндогами и тем самым торнадо-потэйто. наше меню меняется быстрее, чем тренды.

мы постоянно дропаем лимитированные вкусы и коллаборации, которые нужно успеть поймать здесь и сейчас. повторов не будет.

в yoboba полная свобода: заходи с собакой, заряжай телефон, мы не прячем пароль от wi-fi. а для школьников и студентов у нас всегда есть скидки.
