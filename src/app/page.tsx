...
export default async function home_page() {
  ...
  if (!is_supabase_configured()) {
    const from_admin = published?.items?.length
      ? apply_menu_item_badges(published.items)
      : apply_menu_item_badges(default_menu_items);
    return createElement(
      Suspense,
      { fallback: <div className="min-h-screen bg-page" /> },
      createElement(home_client, {
        initial_menu: from_admin,
        initial_categories: published?.categories?.length
        ...
        demo_mode: !published?.items?.length,
      })
    );
  }
  ...
  const initial_menu =
    from_admin ??
    apply_menu_item_badges(resolve_initial_menu(db_menu) ?? default_menu_items);
  const initial_categories = published?.categories?.length
    ? published.categories
    : resolve_menu_categories(initial_menu);
