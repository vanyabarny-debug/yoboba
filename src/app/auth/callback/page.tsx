...
function AuthCallbackInner() {
  ...
  useEffect(() => {
    async function finish_login() {
      ...
      // Supabase иногда отдаёт токены в hash после action_link
            ...
            if (retry.error || !retry.data.session) {
              router.replace(
                `/login?error=${encodeURIComponent(
                  retry.error?.message || error?.message || 'auth_callback_failed'
                )}`
              );
              return;
            }
      ...
      // уже есть сессия (например cookie от предыдущего шага)
